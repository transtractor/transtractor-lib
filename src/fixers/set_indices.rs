...
pub fn fix_set_indices(sd: &mut StatementData) {
    ...
    for (i, proto_transaction) in sd.proto_transactions.iter_mut().enumerate() {
        ...
        let current_date = match proto_transaction.date {
            Some(date) => date,
            None => panic!("Transaction at position {} does not have a date. This should not happen after date fixers.", i),
        };
        
        // Check that dates are in chronological order (panic if not)
        if let Some(prev) = prev_date {
            if current_date < prev {
                panic!("Transaction dates are out of order at position {}: current date {} < previous date {}. The transtractor isn't set up to deal with this.", 
                       i, current_date, prev);
            }
        }
...
mod tests {
    use super::*;
    use crate::structs::{StatementData, ProtoTransaction};
    ...
    #[test]
    fn test_fix_set_indices_same_day_transactions() {
        let mut sd = StatementData {
            proto_transactions: vec![
                create_proto_transaction(1000, 10),
                create_proto_transaction(1000, 20),
                create_proto_transaction(1000, 30),
            ],
            opening_balance: None,
            closing_balance: None,
            start_date: None,
