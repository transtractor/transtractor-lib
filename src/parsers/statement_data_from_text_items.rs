use crate::parsers::statement::OpeningBalanceParser;
...
pub fn parse(config: &StatementConfig, text_items: &TextItems) -> StatementData {
    ...
    let max_lookahead = opening_balance_parser.get_max_lookahead();
    ...
    while i < len {
        ...
        let consumed = opening_balance_parser.parse_items(&buffer, &mut statement_data);
        if consumed > 0 {
            i += consumed;
            continue;
        }
