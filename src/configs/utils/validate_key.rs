use crate::configs::utils::iso_3166_1_alpha_2::is_valid_iso_3166_1_alpha_2;
...
mod tests {
    ...
    #[test]
    fn test_valid_keys() {
        assert!(validate_key("au__cba__credit_card__1").is_ok());
        assert!(validate_key("us__bank__debit__2").is_ok());
        assert!(validate_key("gb__hsbc__loan__10").is_ok());
        assert!(validate_key("de__deutsche__savings__999").is_ok());
    }

    #[test]
    fn test_invalid_uppercase() {
        let result = validate_key("AU__cba__credit_card__1");
    ...
    fn test_invalid_component_count_too_few() {
        let result = validate_key("au__cba__1");
        assert!(result.is_err());
    ...
    fn test_invalid_component_count_too_many() {
        let result = validate_key("au__cba__credit__card__extra__1");
        assert!(result.is_err());
        let err = result.unwrap_err();
        assert!(err.contains("exactly 4 components"));
    ...
    fn test_invalid_country_code() {
        let result = validate_key("xx__cba__credit_card__1");
        assert!(result.is_err());
        let err = result.unwrap_err();
    ...
    fn test_invalid_version_not_integer() {
        let result = validate_key("au__cba__credit_card__abc");
        assert!(result.is_err());
        let err = result.unwrap_err();
    ...
    fn test_invalid_version_empty() {
        let result = validate_key("au__cba__credit_card__");
        assert!(result.is_err());
    ...
    fn test_valid_zero_version() {
        assert!(validate_key("au__cba__credit_card__0").is_ok());
    }

    #[test]
    fn test_valid_negative_version() {
        // Negative integers are still valid integers
        assert!(validate_key("au__cba__credit_card__-1").is_ok());
    }

    #[test]
    fn test_invalid_whitespace_in_key() {
        let result = validate_key("au__cba__credit card__1");
        assert!(result.is_err());
    ...
    fn test_invalid_tab_character() {
        let result = validate_key("au__cba__credit_card\t__1");
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("must not contain whitespace"));
    }
}
